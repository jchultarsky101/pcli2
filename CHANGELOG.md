...
### Fixed
- **A folder match run now stops when the credentials fail, instead of issuing thousands of doomed requests** ([#78](https://github.com/jchultarsky101/pcli2/issues/78)) - When the token expired partway through a long run, automatic re-authentication failed and every remaining asset failed identically: 21,068 of them on one run, each logging its own copy of the same error, taking the full ~25 minutes and ~21,000 pointless API calls to finish. Because the refresh itself has already been attempted by the time the error surfaces, no later request can succeed, so the run now stops at the first such failure. The reason is reported once — not once per asset — and the assets that were never searched are counted separately from the ones that were tried and failed (`Searched 1,259 of 22,378 asset(s): 1 failed, 21,067 not attempted, 51 not searchable`). Only credential failures stop a run; a transient network error or a 5xx on one asset says nothing about the next, so those are reported and the run continues.
