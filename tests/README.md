...
| `reports_test.rs` | The report client (`pcli2 report ...`) against a mock API: the listing sends the type and status filters and stops at `--limit`, a create posts the request and reads the 201 answer, a download streams through a `.part` file and refuses an empty body, get, delete and failure diagnostics use their endpoints. |
