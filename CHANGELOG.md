...
- **`asset counts` adds up** - An asset in a state none of the counters covers (a state added to the API later) was counted in the total and nowhere else. It is now counted as `other` (JSON), in a new last CSV column `OTHER`, and on an "Other state" line of the tree output when there are any.
