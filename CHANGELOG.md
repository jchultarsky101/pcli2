...
- **Excel match reports carry a Summary sheet** - After the matches: tenant, folders, threshold, `--exclusive`, the time and the pcli2 version, then the number of pairs, the assets involved, the number of groups and a distribution of match percentages. A workbook forwarded to someone else used to carry no record of how it was produced.
...
- **`asset list --recursive` lists nested folders correctly** - Below the top level it looked each subfolder up by a path missing its parents (`B/C` instead of `/A/B/C`), so a nested folder was reported missing, or a different top-level folder of the same name was listed instead. It now walks the subtree by folder ID. `--folder-path / --recursive` also includes the assets stored directly at the root, which it used to leave out.
- **`folder download` handles assemblies like `asset download`** - It had its own copy of the assembly handling, without the 2.2.1 fix: an assembly without a dependency bundle failed with a ZIP error. Both commands now share one path. Bundles are extracted from disk instead of being read into memory, entries whose path would leave the download directory are skipped, and the assembly file is written last, so `--resume` never takes a half-extracted bundle for a finished one.
- **`folder download` names the assets it does not download** - Assets that were still indexing or had failed processing were dropped without a word and left out of the totals. Assemblies in `missing-dependencies` are now downloaded. The others are counted in the summary as "Not downloaded" and listed with their state.
- **`folder download` and `folder thumbnail` accept the root** - `--folder-path /` (or `/Home`) failed with "folder not found", although both commands meant to name the output directory after the tenant in that case. They now cover the whole tenant. `folder thumbnail` also checks the folder name before using it as the output directory, and reports an empty folder as a warning rather than an error.
- **`folder get` and `folder rename` explain the root** - `--folder-path /` now says that the root has no folder record of its own (and, for rename, that the root cannot be renamed), instead of "folder not found".

- **JSON reports are byte-identical from run to run** - Asset metadata (in match reports, `asset list`, `asset inventory`) and the health report's file types were written in a different key order on every run, although the folder match help promises identical output. Keys are now written in sorted order, and assets are processed in path order wherever a list is walked. `asset counts` also breaks ties between file types with the same count by name.
- **`asset counts` adds up** - An asset in a state none of the counters covers (a state added to the API later) was counted in the total and nowhere else. It is now counted as `other` in the JSON output and on an "Other state" line of the tree output when there are any. The CSV row is unchanged, so scripts that read it into fixed fields keep working.
- **One very long value no longer loses a whole XLSX report** - A metadata value over Excel's 32,767-character cell limit failed the workbook after the match had finished. It is now cut short and marked `…[truncated]`, with a warning; the CSV and JSON outputs keep the full value.
- **Dependency quantities are reported** - The `OCCURRENCES` column of `asset dependencies` and `folder dependencies` was always 1, and the tree said "(1 occurrences)" for every part: the count the API returns was dropped while the tree was built. A bolt used four times now says 4. An assembly whose data contains itself is expanded once instead of forever.
...
- **Paginated listings never loop, repeat or silently truncate** - One rule now decides when to stop walking a listing: after the last page, at a repeated page number (with a warning), or at the page cap (with a warning that the result is incomplete). The folder tree, which every path lookup reads, had no guard against a repeated page; part search appended the same matches again; the report, metadata-field, missing-metadata and failure listings stopped at 1,000 pages without saying so; `user list` stopped at 5,000 users without saying so.
- **`--limit` no longer repeats or skips records** - `report list`, `tenant failures`, `tenant metadata assets` and `tenant metadata missing` shrank the page size on the last request to fit `--limit` while the page number kept counting in the old size. `--limit 1500` returned records 501-1000 twice and never 1001-1500.
- **`folder upload` and `folder thumbnail` stop cleanly** - Without `--continue-on-error` they returned on the first failure while the remaining uploads carried on in the background, printed no summary, and left the progress bar over the error. They now stop the remaining items, count them as "Not attempted", print the summary and then the failures, as `folder download` does. `folder upload` refuses files that already exist (without `--skip-existing`) before uploading anything, naming all of them, instead of failing when their turn came. `folder thumbnail` writes each image whole or not at all.
...
### Changed
- **`asset metadata create-batch` is faster and never half-applies an unresolvable file** - Every row's asset is looked up first (UUID rows a thousand per request instead of one request each), new fields are registered once, and the rows are then written four at a time (`--concurrent`, 1 to 10; `--concurrent 1` writes strictly in file order). A row that cannot be resolved now stops the batch before anything is written, instead of after the rows above it had been written. Messages, skips and exit codes are otherwise unchanged.
...
### Documentation
- **A shorter README and a book organised by purpose** - The README is down from 1,360 lines to about 200: what pcli2 is, the quick start, installation, logging in, and links. The command and alias reference moved to a Command Reference page, the folder transfer notes to a new "Downloading and Uploading Folders" guide, the pipeline examples to Scripting and Automation, and the upgrade notes to the Installation guide. The book's contents are grouped into Getting Started, Guides, Reference and Help. The site's stale, committed build output (`public/`, last built for 0.2.8) is no longer tracked.
- **Examples you can copy** - Every `pcli2 ...` example in the README and the user guide, and every example in `--help`, is now parsed by a test against the command tree. That caught `pcli2 asset ls` in the top-level help and several README `asset list` examples, which need `--folder-path`; the `jq` example now uses the real field `file_size`, the NuShell examples convert metadata values (which are strings in JSON) before comparing them, and the metadata "export, edit, reimport" workflow builds the `ASSET_PATH,NAME,VALUE` file `create-batch` actually reads.
- **Examples in `--help`** for the most-used commands (`asset get|create|list|download|geometric-match|text-match|dependencies`, `folder list`, `tenant use`, `report list`); the banner shows only above the top-level help; global flags are listed under "Global Options"; help wraps to the terminal width. Examples use `/Home/...` paths (Physna's root) instead of `/Root/...`.
- **Logging in without exposing the secret** - `auth login` reads `PCLI2_CLIENT_ID` and `PCLI2_CLIENT_SECRET`; the documentation and the "not logged in" hints no longer put the secret on the command line, and the GitHub Actions examples pass it through `env:`.
- **New pages**: Credentials and Security (the credentials file is plain text; `auth logout` removes only the token), Proxies, Certificates and Timeouts, and Troubleshooting (by exit code). The quickstart selects a tenant right after logging in. `metadata-inference.md` now says what actually happens on a failure (the run stops at the first failed update). `CONTRIBUTING.md` replaces the documentation-deployment page, which described the wrong trigger.
- **The Dockerfile builds again** - It used Rust 1.83, below the minimum 1.88, and did not account for the library and benchmark targets. The Docker instructions keep the container's configuration in its own directory.
...
- **`asset resolve-dependency` links a missing dependency to an existing asset** (alias `resolve-dep`) - An assembly whose referenced part was not found stays in the `missing-dependencies` state. `--dependency` takes the missing path as `asset dependencies` lists it, `--target-uuid` or `--target-path` the asset that should stand in for it; the server re-indexes the assembly. The dependency must be one of the assembly's missing ones, otherwise the command stops and lists them (exit 64), so a typo never reaches the server. Silent on success, `--dry-run` names everything. Uses `POST /tenants/{tenantId}/assets/{assetId}/resolve-dependency`.

- **`tenant metadata` manages the field registry** - Beside `list`: `rename --name OLD --new-name NEW` (values on assets are kept), `delete --name NAME [--force]` (the server refuses a field still in use unless `--force`, which also removes its values from every asset; confirms unless `--yes`, `--dry-run` supported), `assets --name NAME [--limit N]` (the assets with a value for the field, printed like `asset list`), `coverage` (how many assets carry any metadata: counts and a percentage, JSON or CSV), and `missing [--folder-path P]* [--extension E]* [--limit N]` (the assets with no metadata at all, oldest first; Physna's demo assets excluded). Fields are addressed by their exact registered name; an unknown name lists the registered ones (exit 67). Endpoints: `PATCH`/`DELETE /tenants/{tenantId}/metadata-fields/{fieldId}`, `GET .../metadata-fields/{fieldId}/assets`, `GET .../metadata-coverage`, `GET .../assets/without-metadata`.

- **`pcli2 report` manages report jobs** - A new command group for the server-side reports (duplication, simplification, custom). `report list [--type] [--status] [--limit]` (newest first; CSV `ID,NAME,TYPE,STATUS,PROGRESS,GROUPS,MIN_THRESHOLD,MAX_THRESHOLD,CREATED_AT,UPDATED_AT,CREATOR`), `report get --id`, `report download --id --format csv|xlsx [-o PATH]` (only a COMPLETED report; the file is streamed through a temporary file like an asset download; the default name is the report's name or id plus the extension), `report diagnose --id` (why a report failed, same shape and exit codes as `asset diagnose`), `report delete --id` (confirms unless `--yes`, `--dry-run`), and `report create` for a duplication report over `--folder-path` folders (subfolders included; `--exclude-folder-path`, `--extension`, `--exclude-assemblies`, `--exclude-exact-duplicates`, `--include-home`, thresholds default 80 to 100). `--wait` polls until the report is COMPLETED, FAILED or CANCELLED (progress on stderr; a credential failure stops at once, any other polling error is retried up to three times in a row) and a report that ended FAILED or CANCELLED exits 69 with a pointer to `report diagnose`. `tenant failures` already listed failed reports; their ids now have somewhere to go.

### Changed
- **`tenant metadata list` JSON carries each field's `id`** - The registry listing always sent ids; the client now reads them, since rename and delete address a field by id. The CSV and tree output are unchanged.
